...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (index == 1 || min(parentIndex, index) == parentIndex) {
            return;
        } else {
    ...
    private void sink(int index) {
        ...
        if (getNode(leftIndex) == null && getNode(rightIndex) == null) {
            return;
        } else if (getNode(rightIndex) == null) {
            if (min(index, leftIndex) == leftIndex) {
                swap(index, leftIndex);
                sink(leftIndex);
            }
        } else {
            int smaller = min(leftIndex, rightIndex);
            if (min(smaller, index) == smaller) {
                swap(index, smaller);
                sink(smaller);
            }
        }
    ...
    /* If the array is totally full, resize. */
        ...
        Node newNode = new ArrayHeap.Node(item, priority);
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i <= size; i++) {
            if (item.equals(getNode(i).myItem)) {
                ...
                int parentIndex = ArrayHeap.parentIndex(i);
                if (i == 1) {
                    sink(i);
                } else if (i == size) {
                    swim(i);
                } else {
                    if (min(parentIndex, i) == parentIndex) {
                        sink(i);
                    } else {
                        swim(i);
                    }
